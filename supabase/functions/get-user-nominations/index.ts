...
console.log("get-user-nominations function script started");
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Get the user from the JWT to fetch their specific nominations.
    ...
    // Call the RPC function to get all votes for the user.
    const { data, error } = await supabase.rpc('get_user_votes_for_all_awards', {
      p_user_id: user.id,
    })
    ...
    console.error("Error in get-user-nominations:", error);
