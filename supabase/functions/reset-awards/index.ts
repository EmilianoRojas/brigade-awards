...
console.log("reset-awards function script started");
...
Deno.serve(async (req) => {
    ...
    try {
        ...
        return new Response(JSON.stringify({ message: "Awards reset successfully." }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            status: 200,
        })

    } catch (error) {
        console.error("Error in reset-awards:", error);
