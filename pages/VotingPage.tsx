...
const VotingPage: React.FC<VotingPageProps> = ({ award, onBack }) => {
    ...
    const renderNominationView = () => {
        ...
        return (
            <div>
                <p className="text-gray-300 mb-2">Select up to {award.max_nominations} nominees.</p>
                <p className="text-gray-400 text-sm mb-6">You have selected {selectedNominations.length} / {award.max_nominations}.</p>
                <div className="space-y-4">
                    {candidates.map(candidate => {
                         ...
                         let disabledReason = '';
                         if (isSelf) disabledReason = "You cannot vote for yourself.";
                         else if (isPartner) disabledReason = "You cannot vote for your partner.";
                         else if (hasVoted) disabledReason = "You have already submitted your nominations.";
    ...
    const renderFinalVotingView = () => {
        const hasVoted = !!userVote?.final_vote;
         return (
            <div>
                <p className="text-gray-300 mb-6">Select one nominee to cast your final vote.</p>
                 <div className="space-y-4">
                     {candidates.map(candidate => {
                        const isSelf = candidate.id === user?.id;
                        const isPartner = candidate.id === user?.partner_id;
                        ...
                        else if (hasVoted) disabledReason = "You have already cast your final vote.";
    ...
    const renderResultsView = () => {
        if (results.length === 0) {
            return <p className="text-gray-400">Results are not yet available.</p>;
        }
        const winner = results[0];
        return (
            <div>
                 <div className="bg-gray-800 p-6 rounded-lg mb-8 text-center">
                    <h3 className="text-lg font-medium text-indigo-400">Winner</h3>
                    <div className="mt-4 flex flex-col items-center">
                        ...
                        <p className="text-gray-300">{winner.vote_count} Votes</p>
                    </div>
                </div>
    ...
    const renderContent = () => {
        ...
        return (
            <>
                {hasVoted && <div className="bg-blue-900/50 text-blue-300 p-4 rounded-lg mb-6">Thank you for participating! Your vote for this award has been recorded.</div>}
                ...
                {award.phase === 'CLOSED' && <p className="text-gray-400">This award is currently closed.</p>}
            </>
        )
    };
